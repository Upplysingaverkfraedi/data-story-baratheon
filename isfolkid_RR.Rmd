...
# Notendaviðmót (UI)
ui <- fluidPage(
  titlePanel("Saga Ísfólksins - Líftími, kynja og eiginleiki persóna"),
...
server <- function(input, output) {
  ...
  output$genderAgePlot <- renderPlot({
    ggplot(gender_age_data, aes(x = age, fill = gender)) +
      ...
      theme_minimal() +
      labs(title = "Líftími eftir kyni", x = "Aldur", y = "Fjöldi") +
  ...
  output$ageData <- renderPlot({
    ggplot(age_data, aes(x = gender, y = age, fill = gender)) +
      geom_boxplot() +
      theme_minimal() +
      labs(title = "Líftími eftir kyni - Boxplot", x = "Kyn", y = "Aldur") +
      scale_fill_manual(values = c("F" = "pink", "M" = "lightblue"), name = "Kyn")
  })
    
  # Aldursdreifing good vs evil (Boxplot fyrir líftíma persóna)
    output$ageDistributionPlot <- renderPlot({
      ggplot(age_distribution_data, aes(x = nature, y = age, fill = gender)) +
        geom_boxplot() +
        theme_minimal() +
        labs(title = "Líftími persóna good vs evil með kynjum", x = "Eiginleiki", y = "Aldur") +
        scale_fill_manual(values = c("F" = "pink", "M" = "lightblue"), name = "Kyn")
  })
